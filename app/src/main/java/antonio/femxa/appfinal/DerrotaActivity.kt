...
class DerrotaActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mediaPlayer = MediaPlayer.create(this, R.raw.sonido_perdedor).apply {
            isLooping = false
            setVolume(100f, 100f)
        }
                ...
                viewModel.navigationEvent.collectLatest { event ->
                    when (event) {
                        is NavigationEvent.NavigateToCategoria -> {
                            val intent = Intent(this@DerrotaActivity, CategoriaActivity::class.java)
                            intent.putExtra("SonidoOn-Off", event.musicaOn)
                            startActivity(intent)
                            finish()
                        }
                        is NavigationEvent.NavigateToMenu -> {
                            val intent = Intent(this@DerrotaActivity, InicialActivity::class.java).apply {
                        ...
                        else -> Log.d("DerrotaActivity", "Unhandled navigation event: $event")
                    }
                }
            }
...
fun DerrotaScreen(
    ...
    Surface(
        ...
        Column(
            ...
            Text(text = "¡HAS PERDIDO!", fontSize = 32.sp)
